...
class RpcClientSocket {
    ...
    doConnect(delay) {
        ...
        this.connectTimer = setTimeout(() => {
            let connectCb = function () {
                self.app.logger("debug" /* debug */, `${meFilename} connect to rpc server success: ${self.id}`);
                // register
            ...
            self.app.logger("debug" /* debug */, `${meFilename} try to connect to rpc server: ${self.id}`);
        }, delay);
    }
    onClose() {
        ...
        this.heartbeatTimeoutTimer = null;
        this.socket = null;
        this.app.logger("error" /* error */, `${meFilename} socket closed, reconnect the rpc server later: ${this.id}`);
    ...
    heartbeatTimeoutStart() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
            self.app.logger("error" /* error */, `${meFilename} heartbeat timeout, close the rpc socket: ${self.id}`);
            self.socket.close();
